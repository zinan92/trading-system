...
# 2026-08-04 — Allow immutable Supervisor history through the Cloud gateway (#556)

## Decision

- Add only `/api/trading-system/supervisor-history` to the Cloud Access
  Gateway exact GET allowlist. The Dashboard and Dashboard Server already use
  and implement this route; the gateway omission alone caused the authenticated
  Supervisor tab to receive HTTP 404.
- Retain exact membership checks. Nearby paths, suffixes, prefixes and traversal
  attempts remain denied, and the mutation allowlist and actor verification are
  unchanged.
- Treat Cloud authenticated HTTP 200 with matching cycle identity and complete
  immutable history as release evidence. A green unit test or a local upstream
  response does not complete #556 or #472.

## Gotchas

- The 404 was generated by the gateway allowlist before the request reached the
  Dashboard Server. It did not prove missing Supervisor data and did not affect
  the running Paper strategy.
- The bounded polling read-model intentionally carries only a hash-anchored
  summary. Re-embedding full history there would undo the Dashboard concurrency
  protection and is not an acceptable workaround.
- This read-only route must not be added to `MUTATION_EXACT`; it has no control
  authority and requires no start, stop, cancel, close or runtime restart for
  functional verification.

## Verification

- Focused gateway coverage must prove the exact path is admitted while the same
  path with a trailing slash or child suffix remains fail-closed.
- Existing Dashboard Supervisor browser coverage continues to prove the history
  response schema and complete attempt/start-intent/observation rendering.
