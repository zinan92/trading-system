...
**Overall architecture progress: 87%**
...
It is not yet a full plug-and-play system. Signal analysis and production grid
proposal generation now resolve through separate frozen plugin registries, but
backtest selection still lacks an equivalent port. Several proven compatibility
...
is the arithmetic mean of the seven visible row totals: `610 / 7 = 87.1`,
rounded to `87%`.
...
1. **Backtest composition (medium):** define a Backtest Port over versioned
...
2. **Execution plugin registry (medium):** move the protocol out of
...
5. **Market Envelope V2 cutover (small after evidence):** capture real
...
6. **Legacy read-surface retirement (medium):** migrate remaining consumers to
...
truth again. A new signal-analysis engine or production grid planner can now be
dropped in without editing application/core code. A new backtester cannot yet
make that claim. The honest state is a robust hexagonal spine with completed
signal and proposal plugin boundaries, one weaker upper-layer replay boundary,
and several contained compatibility monoliths.

For the grid-only product focus, the next highest-leverage change is the
Backtest composition—not another Dashboard or provider-specific integration.
