...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build and deliver the single 24-hour trading report.")
    parser.add_argument("--send", action="store_true")
    parser.add_argument("--verify", action="store_true")
    args = parser.parse_args()

    output_root = ROOT / str(load_pipeline_config().get("output_root", "outputs"))
    report_path = TradingDaily24hReportBuilder(output_root).build(now=datetime.now(timezone.utc))
