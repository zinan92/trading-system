...
mutate '09-14 operator stop drops same-tick fills' services/testnet_automation_coordinator.py '        for fill in fills:\n            lifecycle.on_fill(execution_plan, dict(fill), market=market, timestamp=observed_at)' '        for fill in ():\n            lifecycle.on_fill(execution_plan, dict(fill), market=market, timestamp=observed_at)'
mutate '09-14 flattening grid re-enables execution' services/testnet_automation_coordinator.py 'enabled = lifecycle_state != "hard_stop_triggered"' 'enabled = True'
mutate '09-15 hold ignored after take-profit' services/grid_testnet_lifecycle.py 'state.get("status") != "paused_above_range" and not self._rearm_held(state, timestamp=timestamp):' 'state.get("status") != "paused_above_range":'
mutate '09-15 hold ignored on range re-entry' services/grid_testnet_lifecycle.py '                    if not self._rearm_held(state, timestamp=timestamp):\n                        self._rearm_missing_rungs(plan, state, timestamp=timestamp)' '                    if True:\n                        self._rearm_missing_rungs(plan, state, timestamp=timestamp)'
mutate '09-15 unreadable hold file releases' services/grid_testnet_lifecycle.py 'held, detail = True, {"reason": "operator_holds_unreadable", "by": None}' 'held, detail = False, {"reason": "operator_holds_unreadable", "by": None}'
mutate '09-15 release re-arms above the market' services/grid_testnet_lifecycle.py 'if (passive_below is not None and float(rung["price"]) >= passive_below) or (' 'if False and (passive_below is not None and float(rung["price"]) >= passive_below) or ('
mutate '09-15 release never re-arms' services/grid_testnet_lifecycle.py 'if not self._rearm_held(state, timestamp=timestamp) and (was_held or state.get("rearm_pending_after_hold")):' 'if False:'
