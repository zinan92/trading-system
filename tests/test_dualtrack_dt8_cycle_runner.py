...
def test_live_tick_reuses_only_exact_settled_protective_events(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    db = tmp_path / "market_data.db"
                ...
                "positions": [
                    {
                        "status": "open",
                        "remaining_units": 1.0,
                        "entry_ts": "2026-07-05T01:00:30+00:00",
                    }
                ],
                "orders": [],
            }
            ...
            return frozenset(
                {
                    ...
                    f"{cycle_id}:1m:2026-07-05T01:02:00+00:00",
                }
            )
...
def test_live_tick_does_not_reuse_engine_evidence_for_dca_lifecycle(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    db = tmp_path / "market_data.db"
    ...
    class FakeControl:
        def active_plan(self, cycle_id: str) -> dict:
            return {"cycle_id": cycle_id, "strategy_type": "dca"}

        def runtime_state(self, cycle_id: str) -> dict:
            return {
                "cycle_id": cycle_id,
                "strategy_type": "dca",
        ...
        def advance_dca_market_event(
            ...
            *,
            ...
            advanced.append(str(event["event_id"]))
            return {
                "engine_result": {"status": "ok", "triggered": []},
                "state": {"status": "active"},
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: FakeControl(),
    )
