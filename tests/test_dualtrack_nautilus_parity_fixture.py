...
from spikes.dualtrack_nautilus_gold_parity_fixture import (
    SCENARIOS,
    _candidate_orders,
    _exit_event,
    _legacy_fee_config,
...
def test_legacy_parity_uses_the_same_taker_fee_as_candidate() -> None:
    config = _legacy_fee_config({"taker_fee_rate": "0.000400"})

    assert config["cost_per_side_bp"] == 4.0
    assert config["paper_fee_model"]["taker_fee_rate"] == "0.000400"
    assert "execution_cost_model" not in config


def test_legacy_parity_rejects_a_missing_taker_fee() -> None:
    with pytest.raises(ValueError, match="taker_fee_rate"):
        _legacy_fee_config({})


def test_candidate_order_maps_target_to_limit_and_stop_to_market() -> None:
    scenario = _scenario("long_target")
    orders = _candidate_orders(
        [
            {"side": "buy", "event": "entry", "price": 100.0, "quantity": 1.0},
            {"side": "sell", "event": "target", "price": 105.0, "quantity": 1.0},
            {"side": "sell", "event": "stop", "price": 95.0, "quantity": 1.0},
        ],
        scenario,
    )

    assert [order["order_type"] for order in orders] == ["market", "limit", "market"]
