...
class LiveActivationGate:
    ...
    def canary_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            state = None
        if not isinstance(state, Mapping) or state.get("status") != "completed" or state.get("activation_digest") != confirmed.get("activation_digest") or state.get("plan_digest") != confirmed.get("plan_digest") or state.get("state_digest") != _digest({key: value for key, value in state.items() if key != "state_digest"}) or state.get("receipt_chain_digest") != _digest(state.get("receipts") or []) or not state.get("account_snapshot_receipt_digest") or state.get("reconciliation", {}).get("status") not in {"ok", "pass", "reconciled"} or canary.get("state_digest") != state.get("state_digest") or canary.get("receipt_chain_digest") != state.get("receipt_chain_digest") or canary.get("account_snapshot_receipt_digest") != state.get("account_snapshot_receipt_digest") or canary.get("reconciliation_digest") != _digest(state.get("reconciliation") or {}):
