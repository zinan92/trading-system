...
from Math.macd_csv import read_and_calculate_macd,find_crosses,find_max_cross
LINE_TYPE = TypeVar('LINE_TYPE', CBi, "CSeg")


class CSeg(Generic[LINE_TYPE]):
    _data_cache = None  # 类变量，用于缓存数据
    _use_cache = True  # 类变量，控制是否使用缓存

    @classmethod
    def load_data(cls, file_path):
        if cls._use_cache:
            if cls._data_cache is None:
                cls._data_cache = pd.read_csv(file_path, parse_dates=['timestamp'])
    ...
    def __init__(self, idx: int, start_bi: LINE_TYPE, end_bi: LINE_TYPE, is_sure=True, seg_dir=None, reason="normal"):
        ...
        import os
        self.file_path = os.environ.get('CHAN_DATA_DIR', '/tmp/chan_data')
    ...
    def cal_Seg_macd_metric(self, macd_algo):
        if macd_algo == MACD_ALGO.SLOPE:
            return self.Cal_MACD_slope()
        elif macd_algo == MACD_ALGO.AMP:
            return self.Cal_MACD_amp()

        elif macd_algo == MACD_ALGO.AREA:
    ...
    def Cal_MACD_Area(self):
        begin_klu = self.start_bi.get_begin_klu().time.datetime
        end_klu = self.end_bi.get_end_klu().time.datetime
        #csv= '/home/junyu/ec2/BTCUSDT_1m_futures_2024-06-01 00:00:00_2024-10-23 00:00:00.csv'
        #csv='/home/junyu/下载/BTCUSDT_1m_futures_2024-01-01 00:00:00_2024-10-6 00:00:00.csv'
        csvdata = self.load_data(self.end_bi.file_path)

        if self.is_up():
            area,diff,dea=read_and_calculate_macd(csvdata,begin_klu, end_klu, 'up')
            max_cross, cross_type = find_max_cross(csvdata, begin_klu, end_klu, is_up=True)
            all_area=abs(area.sum())
            return all_area,max_cross
        else:
            area,diff,dea=read_and_calculate_macd(csvdata,begin_klu, end_klu, 'down')
            max_cross, cross_type = find_max_cross(csvdata, begin_klu, end_klu, is_up=False)
            all_area=abs(area.sum())
            return all_area,max_cross
