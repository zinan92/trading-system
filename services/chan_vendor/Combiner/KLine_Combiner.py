...
class CKLine_Combiner(Generic[T]):
    ...
    def update_fx(self, _pre: Self, _next: Self, exclude_included=False, allow_top_equal=None,target_type=None):
        # allow_top_equal = None普通模式
        # allow_top_equal = 1 被包含，顶部相等不合并
        # allow_top_equal = -1 被包含，底部相等不合并

        if isinstance(_next, target_type):
            self.set_next(_next)
            self.set_pre(_pre)
            _next.set_pre(self)
            if exclude_included:
            ...
            # elif _pre.high < self.high and _next.high < self.high and _pre.low < self.low and _next.low < self.low and \
            #         _next.low < _pre.low:
            #     self.__fx = FX_TYPE.TOP
            # elif _pre.high > self.high and _next.high > self.high and _pre.low > self.low and _next.low > self.low and \
            #         _next.high > _pre.high:
            #     self.__fx = FX_TYPE.BOTTOM
