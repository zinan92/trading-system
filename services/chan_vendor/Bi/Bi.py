...
from Math.macd_csv import read_and_calculate_macd
...
class CBi:
    ...
    def __init__(self, begin_klc: CKLine, end_klc: CKLine, idx: int, is_sure: bool, coin=None):
        import os
        data_dir = os.environ.get('CHAN_DATA_DIR', '/tmp/chan_data')
        self.file_path = os.path.join(data_dir, f'{coin}.csv') if coin else ''
    ...
    def cal_macd_metric(self, macd_algo, is_reverse=False):
        if macd_algo == MACD_ALGO.AREA:
            return self.Cal_MACD_mandy_Area(is_reverse)
    ...
    def Cal_MACD_mandy_Area(self, is_reverse=False):
        begin_klu = self.begin_klc.time_begin.datetime
        end_klu = self.end_klc.time_end.datetime
        self.data = self.load_data(self.file_path)
        data = self.data
        if self.dir == BI_DIR.UP:
            area, diff, dea = read_and_calculate_macd(data, begin_klu, end_klu, 'up')
            all_area = area.sum()
            return abs(all_area)
        else:
            area, diff, dea = read_and_calculate_macd(data, begin_klu, end_klu, 'down')
            all_area = area.sum()
            return abs(all_area)
    @make_cache
