...
class CKLine_List:
    def __init__(self, kl_type, conf: CChanConfig,coin):
        self.kl_type = kl_type
        self.config = conf
        self.lst: List[CKLine] = []  # K线列表，可递归  元素KLine类型
        self.bi_list = CBiList(bi_conf=conf.bi_conf,coin=coin)
        ...
        self.segsegofsegs_list: CSegListComm[CSeg[CBi]] = get_seglist_instance(seg_config=conf.seg_conf, lv=SEG_TYPE.SEG)

        self.zs_list = CZSList(zs_config=conf.zs_conf)
        self.segzs_list = CZSList(zs_config=conf.zs_conf)
        self.segsegofsegszs_list = CZSList(zs_config=conf.zs_conf)

        self.coin=coin
        #csvdata= '/home/junyu/下载/BTCUSDT_1m_futures_2024-01-01 00:00:00_2024-10-6 00:00:00.csv'
        self.csvdata=None#pd.read_csv(csvdata, parse_dates=['timestamp'])
        self.bs_point_lst = CBSPointList[CBi, CBiList](bs_point_config=conf.bs_point_conf,csvdata=self.csvdata)
        self.seg_bs_point_lst = CBSPointList[CSeg, CSegListComm](bs_point_config=conf.seg_bs_point_conf,csvdata=self.csvdata)
        self.segofsegs_bs_point_lst = CBSPointList[CSeg, CSegListComm](bs_point_config=conf.seg_bs_point_conf,
    ...
    def cal_seg_and_zs(self):
        ...
        self.zs_list.cal_bi_zs(self.bi_list, self.seg_list,self.seg_bs_point_lst)
        ...
        self.segzs_list.cal_bi_zs(self.seg_list, self.segseg_list,self.seg_bs_point_lst)
        update_zs_in_seg(self.seg_list, self.segseg_list, self.segzs_list)  # 计算segseg的zs_lst，以及中枢的bi_in, bi_out

        cal_seg(self.segseg_list, self.segsegofsegs_list)
        self.segsegofsegszs_list.cal_bi_zs(self.segseg_list, self.segsegofsegs_list,self.segofsegs_bs_point_lst)
        update_zs_in_seg(self.segseg_list, self.segsegofsegs_list, self.segsegofsegszs_list)  # 计算segseg的zs_lst，以及中枢的bi_in, bi_out



        # 计算买卖点
        self.seg_bs_point_lst.cal(self.seg_list, self.segseg_list,self.lst,self.bi_list,self.segzs_list)  # 线段线段买卖点
        self.bs_point_lst.bi_cal(self.bi_list, self.seg_list, self.zs_list)  # 再算笔买卖点

        # 高级别递归
        #self.segofsegs_bs_point_lst.cal(self.segseg_list, self.segsegofsegs_list,self.lst,self.bi_list, self.segsegofsegszs_list)

    def need_cal_step_by_step(self):
        return self.config.trigger_step

    def add_single_klu(self, klu: CKLine_Unit):
        timex=klu.time.to_str().split(" ")[0]
        # if timex =='2023/11/14':
        #     print(timex)
        ...
        #         self.last_qushi='UP'
        #     elif self.bi_list[-1].dir==BI_DIR.DOWN:
        #         self.last_qushi='DOWN'

        #
        # print(
        #     klu.time.datetime,klu.high,klu.low,
        #     'klcine: ',
        #     [self.lst[-1].time_begin.to_str(),self.lst[-1].time_end.to_str()] if self.lst else "[]",'bi_list: ',
        #     [self.bi_list[-1].begin_klc.time_begin.to_str(),self.bi_list[-1].end_klc.time_end.to_str(), self.bi_list[-1].dir,self.bi_list[-1].is_sure] if self.bi_list else "[]",'seg_list: ',
        #     self.seg_list[-1] if self.seg_list else "[]",'zs_list: ',
        #     self.zs_list[-1] if self.zs_list else "[]"
        # )
        ...
        if len(self.lst) == 0:
            ...
            if _dir != KLINE_DIR.COMBINE:  # 不需要合并K线
                ...
                if len(self.lst) >= 3:
                    self.lst[-2].update_fx(self.lst[-3], self.lst[-1],exclude_included=False, allow_top_equal=None,target_type=CKLine)
