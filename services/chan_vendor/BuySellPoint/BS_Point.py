...
class CBS_Point(Generic[LINE_TYPE]):
    def __init__(self, bi: LINE_TYPE, is_buy, bs_type: BSP_TYPE, relate_bsp1: Optional['CBS_Point'], feature_dict=None,is_segbsp=False):
        ...
        self.zs = None  # 中枢 一类点有用

        if is_segbsp:
            self.is_segbsp = True


        self.make_sure_time=None
        self.make_sure_bi =None  #  买卖点是过后才确定出来的所以这里的笔的
        self.make_sure_klu = None  # 买卖点是过后才确定出来的所以这里的笔的
    ...
    def add_another_bsp_prop(self, bs_type: BSP_TYPE, relate_bsp1):
        ...
        if self.relate_bsp1 is None:
            ...
            if not  self.relate_bsp1.klu.idx == relate_bsp1.klu.idx:
                print('当前点是一个重合点，且重合点都是前系点的确认点或者2-3类点，他们有不同的参考1类点，',self.relate_bsp1.klu.time, relate_bsp1.klu.time)
