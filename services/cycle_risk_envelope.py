    ...
    def authorize_envelope(
        ...
        *,
        ...
        if kind == "human_explicit":
            ...
            comparisons = _compare_limits(outer["limits"], limits, strategy_type)
...
def _preview_values(preview: Mapping[str, Any], strategy_type: str) -> dict[str, Decimal]:
    ...
    return {
        ...
        "full_depth_loss": _decimal(risk.get("maximum_loss"), "risk_envelope_preview_out_of_bounds"),
...
def _compare_limits(outer: Mapping[str, Any], inner: Mapping[str, Any], strategy_type: str) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for field in (_GRID_FIELDS if strategy_type == "grid" else _DCA_FIELDS):
        outer_value = _decimal(outer.get(field), "outer_strategy_policy_invalid")
