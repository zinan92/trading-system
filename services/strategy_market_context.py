"""Build trusted completed multi-timeframe inputs for grid planning."""
...
def build_strategy_timeframes(
    *,
    ...
    """Return only completed, venue-trusted bars needed by grid sizing."""

    checked_at = parse_utc(as_of)
    feed = feed_cls(market_db=market_db, config=config)
    limits = {"1d": 32, "4h": 64, "1h": 96, "15m": 160}
    seconds = {"1d": 86_400, "4h": 14_400, "1h": 3_600, "15m": 900}
    minimums = {"1d": 15, "4h": 15, "1h": 15, "15m": 50}
    unknown = set(timeframes) - set(limits)
    if unknown:
    ...
    for timeframe in timeframes:
        ...
        trusted = (
            snapshot.get("status") in {"ready", "derived"}
            and snapshot.get("fresh") is True
            and snapshot.get("is_synthetic") is False
        )
        if not trusted:
        ...
        if len(completed) < minimums[timeframe]:
